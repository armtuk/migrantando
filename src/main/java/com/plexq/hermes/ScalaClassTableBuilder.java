...
public class ScalaClassTableBuilder extends TableBuilder {
    ...
    public String buildTable(TableRepresentation tr) throws SQLException, TableBuildException {
        ...
        public String generateCaseClass(TableRepresentation tr) {
            StringBuffer sql = new StringBuffer();
            ...
            for (String a: tr.getTableTypes().keySet()) {
                ...
                sql.append(": ");
                sql.append(tr.getTableTypes().get(a).getSimpleName());
                sql.append(", ");
            }
            sql = trimRight(sql, 2);
            sql.append(")\n\n");
        ...
        public String generateObject(TableRepresentation tr) {
        ...
        public String buildPersistFunction(TableRepresentation tr) {
            TreeMap<String, Class> types = tr.getTableTypes();
            ...
            if (tr.getPrimaryKeys().length == 1 && Arrays.asList(new String[]{"Long","Integer"}).contains(tr.getTypeMap().get(tr.getPrimaryKeys()[0]).getSimpleName())) {
                sql.append("\t\t\tval c = obj.copy(id=DBUtil.nextId)\n\n");
            }
            else {
                sql.append("\t\t\tval c = obj\n\n");
            }
    ...
    public String buildFindByIdFunction(TableRepresentation tr) {
        TreeMap<String, Class> types = tr.getTableTypes();
        StringBuffer sql = new StringBuffer();

        sql.append("\tdef findById(id: ");
        sql.append(tr.getTypeMap().get(tr.getPrimaryKeys()[0]).getSimpleName());
    ...
    public StringBuffer trimRight(StringBuffer s, Integer count) {
        return new StringBuffer(s.substring(0,s.length()-count));
    }
}
